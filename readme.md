...
* hyExt.exe.launchGame 接口参数补充说明
- optParams：可选参数，值为json字符串，目前支持的字段，如下：
  - manualCastScreen： 是否手动投屏到主播端预览区图层(默认值为自动投屏到主播端，手动则需要提示主播捕获相应的窗口或进程)
  - fixedWindowSize：针对云模式下的host窗口，是否支持允许拖动大小(默认值为固定大小，由视频流实际大小显示)
  - exeVersion：针对H5，云模式下的包版本
  - titleName：标题名，针对云模式下的host窗口
  - layerMode：图层布局模式，针对云模式，目前支持三种(0/1/2，即：默认大小图层、全屏预览图层、保持宽高比的全屏预览图层)，默认值为默认大小图层
